...
{
    public sealed partial class TokenCreatePage : AuthPage
    {
        #region Constructors
        ...
        private async void CreateButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (!NeverExpireCheckbox.IsChecked.Value)
            {
                if (expiresSeconds < 0)
                {
                    if (errorText.Length != 0)
                        errorText += "\n";
                    errorText += SetupResources.TokenCreateExpireTimePast;
                }
                else if (expiresSeconds < 60)
                {
                    if (errorText.Length != 0)
                        errorText += "\n";
                    errorText += SetupResources.TokenCreateExpireTimeSoon;
                }
            }
